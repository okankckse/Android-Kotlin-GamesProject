...
import com.okankocakose.appcentproject1.databinding.FragmentViewPagerFirstBinding
...
class ViewPagerFirstFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentViewPagerFirstBinding.inflate(inflater,container,false)
        ...
        apiViewModel.getGamesObserver().observe(viewLifecycleOwner,object : Observer<GamesDataModel?> {
            override fun onChanged(t: GamesDataModel?) {
             if (t==null){
                ...
                Picasso.get().load(t.results.get(0).background_image).into(binding.firstFragmentImageView)
                id= t.results.get(0).id
             }
            }
        })

        binding.firstFragmentImageView.setOnClickListener(object : View.OnClickListener {
